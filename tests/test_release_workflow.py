...
def test_ci_uses_current_flowguard_release() -> None:
    workflow = WORKFLOW.read_text(encoding="utf-8")
    assert workflow.count("FlowGuard.git@v0.69.7") == 1
    assert workflow.count("$env:FLOWGUARD_PROVIDER") == 3
    assert "FlowGuard.git@v0.66.0" not in workflow
    ...
    for job_name in TEST_JOB_NAMES:
        ...
        assert block.index("core.longpaths true") < block.index("$env:FLOWGUARD_PROVIDER")
