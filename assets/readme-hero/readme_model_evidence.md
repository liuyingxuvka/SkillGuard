...
This file records the LogicGuard-backed capability model used for the `v0.3.3` source README update. It is evidence for README synthesis only; it does not replace runnable SkillGuard checks, FlowGuard checks, package publication, GitHub release confirmation, or future AI behavior validation.
...
- release/version facts: the source version is being advanced to `v0.3.3`; `VERSION`, `pyproject.toml`, README, changelog, and release notes must match before any tag or GitHub Release claim.
...
- Target-owned conditional branches, structural `conditional_noop` and `completed_branch` terminals, non-scheduled evidence domains, and direct current-only receipt schemas: `skillguard_v2/contract_schema.py`, `skillguard_v2/native_terminal.py`, `skillguard_v2/closure.py`, the v2 native receipt schemas, and conditional closure/compiler tests.
...
| Conditional branches remain target-owned. | A generic supervisor can accidentally encode one product's branch names, scheduled identity, or no-op semantics. | The current contract declares arbitrary target branch IDs and structural terminal classes; every conditional obligation receives an explicit applicable or not-applicable disposition on every branch. | Compiler/runtime source, v2 native receipt schemas, arbitrary-branch fixtures, and conditional closure tests. | Branch meaning comes from the target contract while SkillGuard verifies total disposition and exact current evidence. | Maintainers can reuse SkillGuard across scheduled and manual workflows without inheriting Khaos-specific branches. | It does not decide which branches a target ought to declare. | The target model and native owner still define product behavior. |
