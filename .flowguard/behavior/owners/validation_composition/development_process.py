...
def build_development_process() -> DevelopmentProcessPlan:
    artifacts = (
        ProcessArtifact(
            ...
            PROCESS_ARTIFACT_REQUIREMENT,
            "bounded-evidence-lifecycle-current",
            path="openspec/changes/add-bounded-evidence-lifecycle",
            owner="openspec",
        ),
        ProcessArtifact(
            ...
            PROCESS_ARTIFACT_DESIGN,
            "bounded-evidence-lifecycle-current",
            path="openspec/changes/add-bounded-evidence-lifecycle/design.md",
            owner="openspec",
        ...
        ProcessArtifact(
            ...
            path=".flowguard/models/owners/validation_composition/validation_composition_model.py",
        ...
        ProcessArtifact(
            ...
            PROCESS_ARTIFACT_FIELD_LIFECYCLE,
            "component-impact-current",
            path=".flowguard/models/owners/validation_composition/field_lifecycle_model.py",
        ...
        ProcessArtifact(
            ...
            PROCESS_ARTIFACT_REPORT,
            "not-run",
            owner="skillguard-evidence-lifecycle",
            upstream_artifact_ids=("report.full-parent",),
        ),
        ProcessArtifact(
            "report.evidence-gc-plan",
            PROCESS_ARTIFACT_REPORT,
            "not-run",
            owner="skillguard-evidence-lifecycle",
            upstream_artifact_ids=("report.evidence-audit",),
        ),
        ProcessArtifact(
            ...
            "not-run",
            owner="skillguard-evidence-lifecycle",
            upstream_artifact_ids=("report.evidence-gc-plan",),
        ),
        ProcessArtifact(
            "report.evidence-pin-replay",
            PROCESS_ARTIFACT_REPORT,
            "not-run",
            owner="skillguard-evidence-lifecycle",
            upstream_artifact_ids=("report.evidence-quarantine", "report.full-parent"),
        ),
        ProcessArtifact(
            "report.evidence-purge",
            PROCESS_ARTIFACT_REPORT,
            "not-run",
            owner="skillguard-evidence-lifecycle",
            upstream_artifact_ids=("report.evidence-pin-replay",),
        ),
    )
    actions = (
        ProcessAction(
        ...
        ProcessAction(
            "audit-evidence-reachability-read-only",
            reads_artifacts=("report.full-parent",),
            writes_artifacts=("report.evidence-audit",),
            order_after=("run-one-admitted-full-parent",),
            status="planned",
            behavior_plane=BCL_PLANE_DEVELOPMENT_PROCESS,
        ),
        ProcessAction(
            "freeze-evidence-gc-plan-read-only",
            reads_artifacts=("report.evidence-audit",),
            writes_artifacts=("report.evidence-gc-plan",),
            order_after=("audit-evidence-reachability-read-only",),
            status="planned",
            behavior_plane=BCL_PLANE_DEVELOPMENT_PROCESS,
        ),
        ProcessAction(
            "quarantine-exact-unreachable-evidence",
            reads_artifacts=("report.evidence-gc-plan",),
            writes_artifacts=("report.evidence-quarantine",),
            order_after=("freeze-evidence-gc-plan-read-only",),
            status="planned",
            behavior_plane=BCL_PLANE_DEVELOPMENT_PROCESS,
        ),
        ProcessAction(
            "replay-current-and-release-pins",
            reads_artifacts=("report.evidence-quarantine", "report.full-parent"),
            writes_artifacts=("report.evidence-pin-replay",),
            order_after=("quarantine-exact-unreachable-evidence",),
            status="planned",
            behavior_plane=BCL_PLANE_DEVELOPMENT_PROCESS,
        ),
        ProcessAction(
            "purge-only-verified-quarantine",
            reads_artifacts=("report.evidence-pin-replay",),
            writes_artifacts=("report.evidence-purge",),
            order_after=("replay-current-and-release-pins",),
            status="planned",
            behavior_plane=BCL_PLANE_DEVELOPMENT_PROCESS,
        ),
    )
