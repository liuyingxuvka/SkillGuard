...
- claim the run before target work and resume only by replaying its contract snapshot and hash-chained events. For every route-conditional run, use the explicit two-stage supervisor: `supervision_mode: stage_depth` with `profiles: []` executes checks and returns status `staged` plus one exact depth receipt; the target then builds/writes its native terminal receipt with `build_target_native_terminal_receipt(...)` and `write_target_native_terminal_receipt(...)`; `supervision_mode: close` resumes the same request/run, reuses the same depth receipt id/hash, consumes the portable terminal ref, and closes without rerunning completed checks. The terminal inherits that receipt's evidence domain; `scheduled_production` alone additionally requires and reverifies the exact scheduled installation identity;
...
Branch-conditional targets must project their own complete branch inventory, conditional obligations, and route/branch requirements. Every conditional obligation is active on at least one branch, verifier-approved as not applicable on at least one branch, and explicitly disposed on every branch. SkillGuard never embeds target branch names or meanings. Native terminal closure binds the exact current declared-check receipt and any verifier-owned applicability receipt. An intermediate authorization remains non-terminal and cannot substitute for the fixed `enforced` terminal completion.
