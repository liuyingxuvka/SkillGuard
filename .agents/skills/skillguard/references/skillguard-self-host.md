...
1. The current compiler checks or regenerates only `.skillguard/compiled-contract.json` and `.skillguard/check-manifest.json` from the FlowGuard model plus confirmed binding source. The declared `check:self:compile-generated-contract` command uses `skillguard_compile.py --check` during validation, so a frozen run never rewrites generated bytes. There is no frozen-old verifier, compatibility reader, or parallel bootstrap stage.
...
When owner execution and aggregation have already completed, finalize from the
two immutable inputs without starting another self-host claim:

```powershell
python .agents/skills/skillguard/scripts/skillguard_self_host.py --repository-root . --finalize --run-root <claimed-run-root> --frozen-plan <run-root>/test-mesh-plan.json --aggregation-ref <aggregation-ref.json> --owner-evidence-root <owner-evidence-root>
```

The finalizer performs one read-only aggregation replay, writes the producer
terminal only if it is new, and atomically publishes the current pointer. A
second invocation with the same terminal is a zero-write no-op.
