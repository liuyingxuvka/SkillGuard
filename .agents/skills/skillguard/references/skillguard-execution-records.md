...
- The receipt binds one exact owner declaration, input-component projection, dependency receipts, target inputs, command/toolchain, environment/verifier, and evidence domain. Run, step, timestamp, parent profile, and display text are attempt or aggregation metadata, not semantic freshness inputs.
