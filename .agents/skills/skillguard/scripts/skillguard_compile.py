...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--repository-root", default=None, help=argparse.SUPPRESS)
    parser.add_argument("--check", action="store_true", help="Check generated parity without writes.")
    args = parser.parse_args(argv)
    repository_root = Path(args.repository_root) if args.repository_root else Path(args.target)
    result = compile_skill_contract(repository_root, write=not args.check)
