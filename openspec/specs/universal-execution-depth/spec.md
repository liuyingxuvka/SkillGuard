# universal-execution-depth Specification

## Purpose
TBD - created by archiving change add-universal-skill-execution-depth. Update Purpose after archive.
## Requirements
...
The Git repository SHALL be the canonical SkillGuard source after direct replacement, and installation SHALL use whole-tree staged copy, validation, backup, and automatic rollback.

#### Scenario: Partial installed sync
- **WHEN** only selected current files are copied into the installed tree
...
- **WHEN** `project-adopt` directly rewrites the SkillGuard block in the current shape
