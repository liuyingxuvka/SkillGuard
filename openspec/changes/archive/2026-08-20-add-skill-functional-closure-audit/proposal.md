...
>
> **Authority status:** historical-only. This directory is not an implementation
> entry point, does not own current schemas, commands, receipts, or tasks, and
> must not be selected by an apply/continue/verify workflow. The sole current
> authority for executable contract and functional-closure work is
> `../build-executable-skill-contract-runtime`.
...
None. The repository has no existing OpenSpec capability specifications. This
historical record does not define current SkillGuard command semantics; any
remaining implementation work belongs to the sole current authority and is a
direct-current replacement or visible blocker, not a current command obligation.
