...
| Deep coverage | Run `check-depth` to verify source requirements, acceptance obligations, skill-specific checks, closure blockers, runtime-owned run records, and non-parallel route proof. |
...
`check-depth` verifies that the contract is not just a shallow entrypoint wrapper. It checks that target-specific requirements are covered by obligations, obligations are tied to checks or native bindings, closure blockers exist, cleanup is not pending, and only `skillguard-runtime` contracts retain an accepted SkillGuard run record bound to the current contract hash.
...
| 深度覆盖 | 用 `check-depth` 检查源要求、验收义务、技能专属检查、关闭阻塞、SkillGuard 自己负责执行时的运行记录和非平行路线证明。 |
...
`check-depth` 用来判断合同是不是只包了一层浅入口。它会检查目标技能的具体要求是否有义务承接，义务是否绑定检查或原生检查，是否存在关闭阻塞，是否还有未完成清理项，以及只有 `skillguard-runtime` 合同才保留已收口并绑定当前合同哈希的 SkillGuard 运行记录。
