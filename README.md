...
| Portfolio calibration | Keep a private ordered registry, inventory every target capability, bind representative jobs to coverage receipts, invalidate old green evidence after Guard changes, permit only history-verifiable proof-bound reuse, and serialize one-target graduation. |
...
issue-portfolio-reuse-ticket, graduate-portfolio,
...
`commands`, `route-task`, `inventory`, `plan-skill`, `generate-skill`, `generate-suite`, `scan-global-skills`, `build-global-registry`, `check-global-registry`, `resolve-global-skill`, `render-global-prompt`, `install-global-prompt`, `check-global-prompt`, `refresh-global-router`, `audit-installed-skills`, `audit-portfolio`, `mark-portfolio-impact`, `issue-portfolio-reuse-ticket`, `graduate-portfolio`, `check-json-schema`, `compile-contract`, `check-contract`, `check-depth`, `check-readme-release`, `select-route`, `start-run`, `advance-run`, `check-run`, `close-run`, `init-target`, `init-suite`, `mark`, `check-skill`, `check-suite`, `check-skill-contract`, `check-suite-map`, `check-suite-contract`, `check-fixture-manifest`, `check-work-contract`, `check-run-record`, `check-check-manifest`, `fixture-test`, `detect-stale-evidence`, `refresh-maintenance`, `review-checker-change`, `check-maintenance-record`, `check-ai-judgment`, `check-report`, `check-workflow-report`, `make-closure`, `self-check`, and `write-report`.

Portfolio commands operate on a private registry outside public target repositories. `mark-portfolio-impact` makes old current entries `revalidation_required`; `issue-portfolio-reuse-ticket` is allowed only for a registered, non-broad, non-intersecting change with unchanged source, contract, command, environment, and coverage identities; `graduate-portfolio` requires a current capability inventory, receipt-bound representative jobs covering every declared capability, current full target evidence, and current proof for every prior active entry. Registry mutations are single-writer and recover abandoned local locks. Excluded private/system skills and supporting repositories remain explicit non-active records.
...
| 组合校准 | 维护私有有序注册表；盘点每个目标的全部能力；把代表任务与覆盖收据绑定；Guard 变化后让旧绿灯失效；只允许可追溯到变更历史的结果复用；串行化逐目标毕业。 |
...
组合命令操作的是位于公开目标仓库之外的私有注册表。`mark-portfolio-impact` 会把旧的 current 条目转成 `revalidation_required`；只有变更已经登记、不是广泛变更、没有功能标签交叉，而且源码、合同、命令、环境和覆盖身份都未变化时，`issue-portfolio-reuse-ticket` 才能复用结果；`graduate-portfolio` 要求能力清单已经完成、代表任务与运行收据的覆盖指纹绑定并覆盖全部声明能力、当前目标具有 full 证据，并要求之前每个活跃条目都有 current 证明。所有注册表变更采用单写入者并能恢复废弃的本机锁。被排除的私人/系统技能和附属仓库继续作为明确的非活跃记录保留。
