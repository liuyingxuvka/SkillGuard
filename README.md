...
Current release: `v0.3.2` (source-only; validation evidence and publication status remain separate claims)
...
```powershell
python .agents/skills/skillguard/scripts/skillguard.py project-adopt --root <repository> --managed-skill "<skill-path>|<native-owner>" --skillguard-version 0.3.2
...
当前发布版本：`v0.3.2`（仅源码；验证证据和发布状态仍是彼此独立的声明）
