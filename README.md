...
Current release: `v0.3.5` (source-only; validation evidence and publication status remain separate claims)

English comes first; the second half is a full Chinese mirror.

SkillGuard is an author-side maintenance and graduation system for Codex skills. It helps a maintainer state what a skill promises, freeze the skill's own checks, verify that the promised work is complete and deep enough, and build a clean consumer distribution. A graduated skill does not need SkillGuard to run.

## Why It Exists

A skill can sound complete while still being shallow: instructions may exist without runnable checks, a declared check may never run, or a broad claim may exceed the evidence. SkillGuard makes those gaps visible during skill maintenance.
...
| Deep verification | Compare declared promises with target-owned checks, current execution evidence, and bounded closure claims. |
...
- fixtures and evidence review: `check-fixture-manifest`, `fixture-test`, `detect-stale-evidence`, `review-checker-change`, `check-maintenance-record`, `check-ai-judgment`, `check-report`, `check-workflow-report`, `make-closure`;
...
A target with one check and a target with many checks use the same supervision rule. Different checks may not be collapsed merely because they invoke the same test program. Different maintenance units never satisfy one another's obligations.
...
3. Run the author-side contract and depth checks.
...
```powershell
python .agents/skills/skillguard/scripts/skillguard.py maintainer-adopt --root <author-repository> --managed-skill "<skill-path>|<native-owner>" --skillguard-version 0.3.5
...
SkillGuard 是一个只在技能维护端使用的“培养与毕业检查系统”。它帮助维护者说明技能承诺什么、冻结技能自己的检查、确认声明的工作是否做完且做得足够深入，然后生成一个干净、独立的消费者版本。技能毕业以后，不需要带着 SkillGuard 才能工作。

## 为什么需要它

技能很容易“文字上完整、实际上很浅”：提示词写得很好，但没有可运行检查；声明了检查，却从未真正运行；或者结论比证据更大。SkillGuard 在维护阶段把这些缺口明确暴露出来。
...
| 深度检查 | 对照技能承诺、目标自己的检查、当前执行证据和有限结论。 |
...
只有一项检查的技能和有很多检查的技能使用同一套监管原则。不能因为多项检查调用了同一个测试程序，就把它们合并成同一项证明。不同维护单元永远不能相互完成义务。

## 常见工作流

### 审计一个技能

先选择明确被维护的源码，阅读它的 `SKILL.md` 和原生检查声明，再运行作者侧合同与深度检查。最后明确报告失败、阻断、跳过、剩余风险和结论边界。
